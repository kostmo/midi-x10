...
options(int argc, char **argv, struct myusbdevice *device) {
  ...
  device->vendor = htoi(argv[1]);
  device->prodid = htoi(argv[2]);
...
main(int argc, char **argv)
{
	...
	if ((iplc = locate_iplc(device)) != NULL) {
		if (usb_detach_kernel_driver_np(iplc, 0) != 0) {
			printf("Error detaching kernel driver.\n");
			exit(1);
		}
		usb_close(iplc);
	} else {
		printf("Unable to locate 0x%04x:0x%04x\n", IPLC_VENDOR, IPLC_PRODUCT);
		exit(1);
	}
	if(device) {
	  free(device);
