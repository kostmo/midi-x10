...
public class CM19a 
{
		...
		public Hashtable<String, ByteBuffer> cache = new Hashtable<String, ByteBuffer>();
		public final int wMaxPacketSize = 8;
		public final String PROTOCOL_FILE = "cm19a.protocol";
		public boolean debug = true; //false;
		public final String PROPERTIES_FILE = "cm19a.properties";
		public final String debugKey = "cm19a.debug";
		public LinkedList<byte[]> outQueue = null;


		public CM19a() {
				outQueue = new LinkedList<byte[]>();
		}
		...
		public void loadProperties() {
				...
				if ( file != null ) {
						...
						String debugOn = prop.getProperty( debugKey, "true" );
		...
		public UsbDevice findDevice(UsbHub hub) {
				...
				if ( debug ) System.out.println("Hub found; iterating through ports");
				if ( debug && hub.isRootUsbHub() ) System.out.println( "This is the virtual root hub");
				...
				while ( iterator.hasNext() ) {
						...
						if ( port.isUsbDeviceAttached() ) {
								if ( debug ) System.out.println( "There is a usb device attached to the port" );
								UsbDevice device = port.getUsbDevice();
								if ( device == null && debug ) System.out.println( "..no, its null" );
								/* Is this the device? If so, quit looking 
									 ...
									 if ( debug ) System.out.println( " This device is a hub ");
									 cm19a = findDevice( (UsbHub)device );
									 if ( isDevice( cm19a ) ) break;
										
									 } else if ( debug ) System.out.println( "This device is not the device we're looking for and its not a hub" ); */
		...
		public void startDriver() throws CM19aException, UsbException, UnsupportedEncodingException
		{
				...
				if (cm19a == null ) {
						...
						if ( debug ) {
							...
							// XXX This method doesn't always work; sometimes it says:
							// "Unable to get string descriptor languages: LIBUSB_ERROR_TIMEOUT".
								try {
		...
		public void releaseDevice() throws CM19aException, UsbException {
				...
				// XXX This was crashing with an "IllegalMonitorStateException":
//				outQueue.notify();
...
//				public synchronized void quit() throws UsbException {
		...
		public static void main (String argv[])
		{
				...
				System.out.println("Running the depth-first debug version of the driver");
				/*Start the driver*/
				try {
